...
Classificador = MLPClassifier(hidden_layer_sizes = (190), alpha=1, max_iter=100)
...
ClassificadorPCA = MLPClassifier(hidden_layer_sizes = (550), alpha=1, max_iter=1500)
...
ConfusionMatrixDisplay.from_estimator(Classificador, features, target,include_values=True,display_labels=data.target_names)
plt.title("Gráfico s/ PCA")
plt.show()

ConfusionMatrixDisplay.from_estimator(ClassificadorPCA, pca_features, target,include_values=True,display_labels=data.target_names)
plt.title("Gráfico c/ PCA")
plt.show()